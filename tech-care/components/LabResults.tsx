...
export default function LabResults({ results = [] }: LabResultsProps) {
  const defaultResults: LabResult[] = results.length > 0 ? results : [
    {
      id: '1',
      name: 'Blood Test',
      date: 'Dec 15, 2025'
  ...
  return (
    <div className="p-6 flex-1 overflow-hidden flex flex-col">
      <h2 className="text-xl font-bold text-gray-800 mb-6 shrink-0">Lab Results</h2>
        ...
        {defaultResults.map((result) => (
          ...
          >
            ...
            <button className="p-2 hover:bg-gray-200 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
              <span className="w-5 h-5 block text-gray-600">[Download Icon]</span>
            </button>
          </div>
        ))}
      </div>
